...
import randomForest_Global_d2 as modelfncs
...
# Save the figure
tmp_image = os.path.join(tmp_image_direct, "learning_curve.png")
...
# Example lane to visualize
example_lane = all_traces[0]
trace_len = len(example_lane)
print("Trace length (number of vertical pixels):", trace_len)

# Downsample length used in extract_features
down_len = 100
importance_down = clf.feature_importances_[-down_len:]  # last 100 features

# Upsample to match original trace length
importance_up = np.interp(
    np.arange(trace_len),
    np.linspace(0, trace_len-1, down_len),
    importance_down
)
...
# Left y-axis: Feature importance (0-1)
ax1.plot(importance_up, color='red', alpha=0.8, label='Feature importance')
ax1.set_ylabel('Feature importance', color='red')
ax1.set_ylim(0, max(importance_up)*1.2)  # give a bit of padding
...
plt.title("Gel lane with feature importance overlay")
...
# Save and show
feature_imp_file = os.path.join(tmp_image_direct, "feature_importance_dualaxis_GlobalWeight.png")
