...
print("Using multiple L2 values:", l2_values)
...
for l2_val in l2_values:
    ...
    print(f"Running CNN with L2 = {l2_val}...")
    for frac in train_fracs:
        ...
        cnn_tmp = Sequential([
            Conv1D(32, kernel_size=5, activation='relu', input_shape=X_train_cnn.shape[1:]),
            MaxPooling1D(2),
            Conv1D(64, kernel_size=5, activation='relu'),
            MaxPooling1D(2),
            Flatten(),
    ...
    # Save results for this L2
    cnn_results['L2'].append(l2_val)
    cnn_results['TrainAcc'].append(cnn_train_scores[-1])
    cnn_results['ValAcc'].append(cnn_val_scores[-1])

# Convert to DataFrame for easy comparison
df_cnn_compare = pd.DataFrame(cnn_results)
df_cnn_compare
...
for l2_val in l2_values:
    # Load the saved CNN for this L2 (or retrain if not saved)
    cnn_tmp = Sequential([
        Conv1D(32, kernel_size=5, activation='relu', input_shape=X_train_cnn.shape[1:]),
        MaxPooling1D(2),
        Conv1D(64, kernel_size=5, activation='relu'),
    ...
    # Train briefly (or use your learning curve results / saved model)
    cnn_tmp.fit(
        X_train_cnn, y_train_cnn,
        validation_data=(X_val_cnn, y_val_cnn),
        epochs=25,
    ...
    # Predictions
    y_val_pred_cnn_probs = cnn_tmp.predict(X_val_cnn)
    ...
    df_tmp = pd.DataFrame(report_cnn).transpose()
    
    # Add overall accuracy
    df_tmp.loc['accuracy', :] = np.nan  # create row for accuracy
    df_tmp.at['accuracy', 'precision'] = (y_val_pred_cnn == y_val_true_cnn).mean()
    df_tmp.at['accuracy', 'recall'] = np.nan
    df_tmp.at['accuracy', 'f1-score'] = np.nan
    df_tmp.at['accuracy', 'support'] = len(y_val_true_cnn)
    
    df_tmp["Model"] = f"1D CNN L2={l2_val}"
    df_cnn_list.append(df_tmp)

# Combine all CNNs
df_cnn = pd.concat(df_cnn_list, ignore_index=True)
...
df_compare[numeric_cols] = df_compare[numeric_cols].round(2)

print(df_compare)

# Optionally save
compare_file = os.path.join(tmp_image_direct, "Model_Comparison_RF_CNN_multipleL2.csv")
